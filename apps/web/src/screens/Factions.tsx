...
export default function Factions({ player, onBack }: Props) {
  ...
  return (
    <div style={{ display:'grid', gap:12, padding:16 }}>
      <Header title={T.buttons?.guild ?? 'Factions'} gold={player.gold} energy={player.energy} level={player.progress.level} onBack={onBack} />

      <section style={{ padding:12, borderRadius:16, background:'var(--panel-bg)', border:'var(--panel-border)', display:'grid', gap:10 }}>
        <div style={{opacity:.9}}>{(T.factions?.yours ?? 'Your faction')}: <b>{mine ? (list.find(x=>x.id===mine)?.name || mine) : (T.factions?.none ?? 'None')}</b></div>
        {mine && <Button onClick={leave}>{T.factions?.leave ?? 'Leave faction'}</Button>}
      </section>
        ...
        {list.map(f => (
          <div key={f.id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 12px', borderRadius:12, background:'var(--panel-bg)', border:'1px solid rgba(255,255,255,0.08)'}}>
            <div>
              ...
              <div style={{opacity:.85, fontSize:12}}>{(T.factions?.members ?? 'Members')}: {f.members}</div>
            </div>
            <Button onClick={() => join(f.id)} disabled={mine===f.id}> {mine===f.id ? (T.factions?.joined ?? 'Joined') : (T.factions?.join ?? 'Join')} </Button>
          </div>
        ))}
      </section>
    </div>
