...
## TODO: Визуальный редизайн (после готовности функционала)

- Базовая тема (CSS‑переменные)
  - Обновить палитру/тени/радиусы в `apps/web/src/styles/theme.css` (`:root`).
  - Минимизировать инлайновые стили: компоненты читают токены (`var(--...)`).

- Фоны и оформление
  - Добавить фон(ы) страницы: `apps/web/src/assets/ui/bg_main.webp`, подключить в теме/на Home.
  - Проверить контраст текста/кнопок поверх фона.

- Кнопки/иконки/рамки
  - Кнопка (`apps/web/src/components/Button.tsx`): фон/бордер/hover/active через переменные темы.
  - Иконки SVG/PNG в `apps/web/src/assets/icons/`; согласовать размеры и стиль.

- Изображения контента
  - Монстры: поле `image` у `MONSTERS` (`apps/web/src/store/battle.ts`), ассеты `apps/web/src/assets/monsters/` (WebP/PNG, ~256×256).
  - Предметы: поле `image` в каталоге (`apps/web/src/data/catalog.ts`), ассеты `apps/web/src/assets/items/` (PNG/WebP, 64–96 px).
  - Портреты классов: обновить `CLASS_ASSETS` (папка `apps/web/src/assets/classes/`).

- Гайд по ассетам
  - Формат: WebP/PNG (иконки — SVG), нейминг: `ui/bg_main.webp`, `monsters/boar.webp`, `items/warrior_helmet_t1.png`.
  - Ограничения по весу: фон ≤ 500 KB, иконки ≤ 50 KB.

- Дополнительно (опционально)
  - Переключатель темы (тёмная/светлая) на токенах.
  - Небольшие анимации появлений (CSS keyframes) для карточек/кнопок.
